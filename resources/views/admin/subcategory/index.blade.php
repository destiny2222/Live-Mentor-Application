                        ...
                        <tbody>
                            @foreach($subcategory as $key => $categories)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.subcategory.edit', $categories->id) }}" class="btn btn-primary btn-sm">Edit</a> &nbsp;
                                        <a href="{{ route('admin.subcategory.delete', $categories->id) }}" onclick="event.preventDefault(); document.getElementById('delete-{{ $categories->id }}').submit();" class="btn btn-danger btn-sm">Delete</a>
                                        <form action="{{ route('admin.subcategory.delete', $categories->id) }}" id="delete-{{ $categories->id }}" method="post" style="display:none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
